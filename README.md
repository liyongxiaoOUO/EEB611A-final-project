...
Autonomous Patrol: Navigates the environment while avoiding obstacles using LiDAR sensor fusion.

Fire Detection: Uses HSV color thresholding to detect thermal anomalies (red objects).

Active Response: Performs a "wiggling" maneuver to simulate extinguishing upon detection.

Auto-Evidence Logging: Automatically saves timestamped images and updates a mission log when a hazard is found.

Smart Escape: Implements a remote "RESUME" command with a blind-turn maneuver to prevent re-detection loops.
